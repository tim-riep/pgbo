...
export interface ActionDef<TData = any> {
  ...
  /**
   * Optional JSON Schema describing the action's request body. Picked up by
   * `@pgbo/fastify` to populate the route's `body` schema (issue #38), so the
   * generated OpenAPI spec documents the action's expected input.
   *
   * Pass any JSON Schema 7 shape, e.g. `{ type: 'object', properties: { id: { type: 'integer' } }, required: ['id'] }`.
   * Without `inputSchema`, the body schema falls back to `additionalProperties: true`.
   */
  inputSchema?: Record<string, unknown>
  /**
   * Optional human-readable summary for the action's HTTP route. Surfaces in the
   * OpenAPI spec generated by `@fastify/swagger`.
   */
  summary?: string
  /**
   * Optional human-readable description for the action's HTTP route.
   */
  description?: string
}
